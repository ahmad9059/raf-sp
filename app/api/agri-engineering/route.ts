...
// GET - Fetch all Agri Engineering assets
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch Agricultural Engineering assets" }, { status: 500 });
  }
}

// POST - Create a new Agri Engineering asset
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedFields = agriEngineeringAssetSchema.safeParse(body);
    ...
    return NextResponse.json({ error: "Failed to create Agricultural Engineering asset" }, { status: 500 });
  }
}
