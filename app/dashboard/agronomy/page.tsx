...
import {
  useAgronomyEquipment,
  useDeleteAgronomyEquipment,
  type AgronomyEquipment,
} from "@/hooks/use-agronomy";
import { AgronomyEquipmentTable } from "@/components/agronomy/agronomy-equipment-table";
import { AgronomyEquipmentForm } from "@/components/agronomy/agronomy-equipment-form";
...
export default function AgronomyDashboardPage() {
  ...
  const [selectedEquipment, setSelectedEquipment] = useState<AgronomyEquipment | null>(null);

  // Fetch Agronomy equipment
  const { data: equipment, isLoading, error, refetch } = useAgronomyEquipment();

  // Delete mutation
  const deleteMutation = useDeleteAgronomyEquipment();

  const handleEdit = (item: AgronomyEquipment) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">
            Agronomy Department Equipment
          </h1>
          <p className="text-gray-600 mt-1 text-sm lg:text-base">
            Manage Agronomy Department laboratory equipment and inventory
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            View and manage all Agronomy department equipment. Use the search and filters to find specific items.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <LoadingState
