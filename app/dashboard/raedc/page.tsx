...
import { useRAEDCEquipment, useDeleteRAEDCEquipment, type RAEDCEquipment } from "@/hooks/use-raedc";
import { RAEDCTable } from "@/components/raedc/raedc-table";
import { RAEDCForm } from "@/components/raedc/raedc-form";
...
export default function RAEDCDashboardPage() {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedEquipment, setSelectedEquipment] = useState<RAEDCEquipment | null>(null);

  // Fetch RAEDC equipment
  const { data: equipment, isLoading, error, refetch } = useRAEDCEquipment();

  // Delete mutation
  const deleteMutation = useDeleteRAEDCEquipment();

  const handleEdit = (item: RAEDCEquipment) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">
            RAEDC Department Equipment
          </h1>
          <p className="text-gray-600 mt-1 text-sm lg:text-base">
            Manage Regional Agricultural Extension and Development Center facilities and equipment
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            View and manage all RAEDC department equipment and facilities. Use the search and filters to find specific items.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <LoadingState
