...
import { useMRIAssets, useDeleteMRIAsset, type MRIAsset } from "@/hooks/use-mri";
import { MRIAssetTable } from "@/components/mri/mri-asset-table";
import { MRIAssetForm } from "@/components/mri/mri-asset-form";
...
export default function MRIDashboardPage() {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [selectedAsset, setSelectedAsset] = useState<MRIAsset | null>(null);

  // Fetch MRI assets
  const { data: assets, isLoading, error, refetch } = useMRIAssets();

  // Delete mutation
  const deleteMutation = useDeleteMRIAsset();

  const handleEdit = (asset: MRIAsset) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-gray-900">
            MRI Department Assets
          </h1>
          <p className="text-gray-600 mt-1 text-sm lg:text-base">
            Manage Mango Research Institute assets and inventory
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            View and manage all MRI department assets. Use the search and filters to find specific items.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <LoadingState
