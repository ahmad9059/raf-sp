...
import type { MNSUAMFacility } from "@/hooks/use-mnsuam";

interface MNSUAMFacilityTableProps {
  data: MNSUAMFacility[];
  onEdit: (facility: MNSUAMFacility) => void;
...
export function MNSUAMFacilityTable({ data, onEdit, onDelete }: MNSUAMFacilityTableProps) {
  ...
  const columns: ColumnDef<MNSUAMFacility>[] = [
    {
      accessorKey: "name",
      header: "Name",
      cell: ({ row }) => (
    ...
    },
    {
      ...
      header: "Block",
      cell: ({ row }) => (
        <div className="max-w-[100px] truncate" title={row.getValue("blockName") || ""}>
    ...
    },
    {
      ...
      cell: ({ row }) => <div>{row.getValue("facilityType") || "-"}</div>,
    },
    {
      accessorKey: "capacityPersons",
      header: "Capacity",
      cell: ({ row }) => {
        const capacity = row.getValue("capacityPersons") as number | null;
        const label = row.original.capacityLabel;
        if (capacity) {
          return <div>{String(capacity)} {label || "persons"}</div>;
        }
    ...
    },
    {
      ...
      header: "Status",
      cell: ({ row }) => {
        const status = row.getValue("status") as MNSUAMFacility["status"];
